...
.toc a:hover span,.toc a:focus-visible span{opacity:1;transform:translate(0,-50%)}
...
</main><footer><div class="wrap">Prepared by Mohan Kumar Allada for the TransPerfect Paybooks SEO proposal · 24 September 2026 · Data: Ahrefs, competitor sitemaps, live page measurement</div></footer><script>(function(){{var L=[].slice.call(document.querySelectorAll('.toc a'));var S=L.map(function(a){{return document.querySelector(a.getAttribute('href'))}});function f(){{var y=window.scrollY+window.innerHeight*0.35,k=0;S.forEach(function(e,i){{if(e&&e.offsetTop<=y)k=i}});L.forEach(function(a,i){{a.classList.toggle('on',i===k)}})}}window.addEventListener('scroll',f,{{passive:true}});f()}})();</script></body></html>'''
