...
(function () {
  ...
  // 1. Styles: no selection, no drag, no print, watermark, blur state
  var css =
    ...
    '.pz-wm{position:fixed;inset:0;z-index:2147483645;pointer-events:none;opacity:.05;background-repeat:repeat;background-size:520px 300px}' +
    '@media print{html,body{display:none!important;visibility:hidden!important}}';
  var st = d.createElement('style'); st.textContent = css; d.head.appendChild(st);

  // 2. Watermark (tiled, diagonal, very faint)
  var svg = "<svg xmlns='http://www.w3.org/2000/svg' width='520' height='300'><text x='260' y='150' text-anchor='middle' transform='rotate(-22 260 150)' font-family='Inter,Arial,sans-serif' font-size='18' font-weight='700' fill='%230E1411'>CONFIDENTIAL · PAYBOOKS PROPOSAL</text></svg>";
  function addLayers() {
    if (!d.body || d.querySelector('.pz-wm')) return;
    var wm = d.createElement('div'); wm.className = 'pz-wm';
    wm.style.backgroundImage = 'url("data:image/svg+xml;utf8,' + svg.replace(/#/g, '%23') + '")';
    var sh = d.createElement('div'); sh.className = 'pz-shield';
    sh.textContent = 'Content protected. Click here to continue reading.';
    d.body.appendChild(wm); d.body.appendChild(sh);
  }
  if (d.body) addLayers(); else d.addEventListener('DOMContentLoaded', addLayers);

  // 3. Block copy, cut, paste-out, selection, drag and right-click
  ...
  // 4. Block shortcuts: copy, cut, select all, save, print, view source, dev tools
  ...
  // 5. Blur when the window loses focus (screenshot tools, screen share, switching apps)
  ...
  // 6. Block printing from the browser menu
